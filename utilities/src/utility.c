 ...
 *******************************************************************************
 ...
 reads a file, assuming it consists of a certain number of columns,
 and splits the columns into input and output columns according to
 a sorting index (optional). Every file is also duplicated as a 
 binary file, and the binary file is updated whenever the date of
 the original file is newer than that of the binary file

 Note: the arrays for input and output are allocated by this function
 according to my matrix conventions
 
 *******************************************************************************
 Function Parameters: [in]=input,[out]=output
 
 \param[in]     fname    : the filename (without the .bin suffix)
 \param[in]     n_col    : number of columns
 \param[in]     n_in     : number of inputs
 \param[in]     n_out    : number of outputs
 \param[in]     ind_in   : index array to map columns to inputs (pass NULL to do 1 to 1)
 \param[in]     ind_out  : index array for outputs (analogously as ind_in)
 \param[out]    X        : input data
 \param[out]    Y        : output data
 \param[out]    nrow     : number of rows read
 
 ******************************************************************************/
int
read_file_bin(char *fname, int n_col, int n_in, int n_out, int *ind_in,
	      int *ind_out, double ***X, double ***Y, int *nrow)

{
  ...
  FILE   *fp;
  FILE   *fpbin;
  int    create_bin_file = FALSE;
  int    ndata=0;
  double dummy;
  double **data;
  int    ind_in_flag = FALSE;
  int    ind_out_flag = FALSE;
  int    rc;
  
  /* does the file exist in .bin mode? */

  sprintf(string,"%s.bin",fname);
  fpbin = fopen(string,"r");
  fp    = fopen(fname,"r");
  ...
  if (fp != NULL && fpbin != NULL) {
    ...
    if (s.st_mtime > sbin.st_mtime) {
      create_bin_file = TRUE;
      fclose(fpbin);
    } else {
      fclose(fp);
    }
    ...
    /* something is wrong; I do not accept pure .bin files or
       missing stuff */
...
AGAIN:
  ...
  /* now sort out what we have to do */

  if (create_bin_file) {

    /* run through the file to count the number of lines */

    ndata = 0;
    for (;;) {
      if (fscanf(fp,"%lf",&dummy) == EOF)
	break;
      ++ndata;
    }
    ...
    if (ndata%n_col != 0) {
      printf("Number of columns in file >%s< does not seem to be %d\n",
	     fname,n_col);
      fclose(fp);
      return FALSE;
    }
    ...
    data = my_matrix(1,ndata,1,n_col);

    for (i=1; i<=ndata; ++i) {
      for (j=1; j<=n_col; ++j) {
	rc = fscanf(fp,"%lf",&data[i][j]);
      }
    }
    ...
    /* read the first line of the .bin file */

    iaux = n_col;
    rc = fscanf(fpbin,"%d %d",&ndata,&n_col);
    if (iaux != n_col) {
      printf("Number of columns does not match file: %d != %d\n",
	     iaux,n_col);
      fclose(fpbin);
      return FALSE;
    }
    ...
    data = my_matrix(1,ndata,1,n_col);
    fgetc(fpbin);
    if (!fread_mat(fpbin,data)) {
      /* this might be a binary incompatibility -- just create a new bin file */
      my_free_matrix(data,1,ndata,1,n_col);
      create_bin_file = TRUE;
      fp = fopen(fname,"r");
      fclose(fpbin);
      goto AGAIN;
  ...
  /* now copy the data into the appropriate columns of the X and Y
     matrices */

  if (n_in > 0 && ndata > 0) 
    XX = my_matrix(1,ndata,1,n_in);
  else 
    XX = NULL;
  if (n_out > 0 && ndata > 0)
    YY = my_matrix(1,ndata,1,n_out);
  else
    YY = NULL;
  if (ind_in == NULL && n_in > 0) {
    ind_in = my_ivector(1,n_in);
    for (i=1; i<=n_in; ++i) ind_in[i]=i;
    ind_in_flag = TRUE;
  }

  if (ind_out == NULL && n_out > 0) {
    ind_out = my_ivector(1,n_out);
    for (i=1; i<=n_out; ++i) ind_out[i]=i+n_in;
    ind_out_flag = TRUE;
  }
  ...
  for (i=1; i<=ndata; ++i) {
    for (j=1; j<=n_in; ++j)  XX[i][j] = data[i][ind_in[j]];
    for (j=1; j<=n_out; ++j) YY[i][j] = data[i][ind_out[j]];
  }

  my_free_matrix(data,1,ndata,1,n_col);
  if (ind_in_flag) my_free_ivector(ind_in,1,n_in);
  if (ind_out_flag) my_free_ivector(ind_out,1,n_out);
