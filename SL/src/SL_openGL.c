...

{
  ...
  SL_quat robotOrient;
  robotOrient.q[_Q0_] = 1;
  robotOrient.q[_Q1_] = 0;
  robotOrient.q[_Q2_] = 0;
  robotOrient.q[_Q3_] = 0;

  double robotPos[4] = {0, 0, 0, 0};

  /* Draw the global axes (see keys) */
  if (ptr->draw_axis) {
    drawCoord(robotPos, &robotOrient, 100);
  }
  ...
  if (ptr->draw_axis) {
	    drawCoord(robotPos, &robotOrient, 0.25);
  }
...
draws a coordinate System at 'pos' with orientation 'orient' given as a quaternion and an axis length of 'axisLength'


******************************************************************************/
void drawCoord(Vector pos, SL_quat *orient, double axisLength){
	double xAxis[4], yAxis[4], zAxis[4];

	xAxis[_X_] = (pow(orient->q[_Q0_],2) +pow(orient->q[_Q1_],2) -pow(orient->q[_Q2_],2) -pow(orient->q[_Q3_],2))*axisLength;
	xAxis[_Y_] = (2*orient->q[_Q1_]*orient->q[_Q2_] -2*orient->q[_Q0_]*orient->q[_Q3_])*axisLength;
	xAxis[_Z_] = (2*orient->q[_Q0_]*orient->q[_Q2_] +2*orient->q[_Q1_]*orient->q[_Q3_])*axisLength;

	yAxis[_X_] = (2*orient->q[_Q0_]*orient->q[_Q3_] +2*orient->q[_Q1_]*orient->q[_Q2_])*axisLength;
	yAxis[_Y_] = (pow(orient->q[_Q0_],2) -pow(orient->q[_Q1_],2) +pow(orient->q[_Q2_],2) -pow(orient->q[_Q3_],2))*axisLength;
	yAxis[_Z_] = (2*orient->q[_Q2_]*orient->q[_Q3_] -2*orient->q[_Q0_]*orient->q[_Q1_])*axisLength;

	zAxis[_X_] = (2*orient->q[_Q1_]*orient->q[_Q3_] -2*orient->q[_Q0_]*orient->q[_Q2_])*axisLength;
	zAxis[_Y_] = (2*orient->q[_Q0_]*orient->q[_Q1_] +2*orient->q[_Q2_]*orient->q[_Q3_])*axisLength;
	zAxis[_Z_] = (pow(orient->q[_Q0_],2) -pow(orient->q[_Q1_],2) -pow(orient->q[_Q2_],2) +pow(orient->q[_Q3_],2))*axisLength;

	glDisable(GL_LIGHTING); /*to have constant colors */
	glBegin(GL_LINES);
		glColor4fv (SL_darkRed);
		glVertex3d(pos[_X_]-xAxis[_X_],pos[_Y_]-xAxis[_Y_],pos[_Z_]-xAxis[_Z_]);
		glVertex3d(pos[_X_],pos[_Y_],pos[_Z_]);
		glColor4fv (SL_red);
		glVertex3d(pos[_X_],pos[_Y_],pos[_Z_]);
		glVertex3d(pos[_X_]+xAxis[_X_],pos[_Y_]+xAxis[_Y_],pos[_Z_]+xAxis[_Z_]);
	glEnd();
	glBegin(GL_LINES);
		glColor4fv (SL_darkGreen);
		glVertex3d(pos[_X_]-yAxis[_X_],pos[_Y_]-yAxis[_Y_],pos[_Z_]-yAxis[_Z_]);
		glVertex3d(pos[_X_],pos[_Y_],pos[_Z_]);
		glColor4fv (SL_green);
		glVertex3d(pos[_X_],pos[_Y_],pos[_Z_]);
		glVertex3d(pos[_X_]+yAxis[_X_],pos[_Y_]+yAxis[_Y_],pos[_Z_]+yAxis[_Z_]);
	glEnd();
	glBegin(GL_LINES);
		glColor4fv (SL_darkBlue);
		glVertex3d(pos[_X_]-zAxis[_X_],pos[_Y_]-zAxis[_Y_],pos[_Z_]-zAxis[_Z_]);
		glVertex3d(pos[_X_],pos[_Y_],pos[_Z_]);
		glColor4fv (SL_blue);
		glVertex3d(pos[_X_],pos[_Y_],pos[_Z_]);
		glVertex3d(pos[_X_]+zAxis[_X_],pos[_Y_]+zAxis[_Y_],pos[_Z_]+zAxis[_Z_]);
