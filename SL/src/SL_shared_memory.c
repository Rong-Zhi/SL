...
SEM_ID             sm_robot_message_ready_sem; /* signals message to motor servo */
...
smMessage         *sm_robot_message;
SEM_ID             sm_robot_message_sem;
...
init_shared_memory(void)
{
  ...
  /********************************************************************/
  if (init_sm_object("smRobotMessage", 
                     sizeof(smMessage),
                     0,
                     &sm_robot_message_sem,
                     (void **)&sm_robot_message)) {
    ;
  } else {
    return FALSE;
  }
  ...
  if (!init_sm_sem("smRobotMsgReadySem", SEM_EMPTY,(void**)&sm_robot_message_ready_sem))
    return FALSE;
		   ...
		   void *buf, int n_bytes) 
{
 ...
 void
   sendMessageRobotServo(char *message, void *buf, int n_bytes)
 {
   sendMessageToServo(sm_robot_message, sm_robot_message_sem, 
                      sm_robot_message_ready_sem,
                      message, buf, n_bytes);
 }
