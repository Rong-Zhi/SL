...
removeSharedMemory(int dummy)
{
  ...
  /**
   * Fix logging when SL is run with full path,
   * i.e. `/bla/bla/xbarret` instead of `./xbarret`.
   *
   * In this case, `servo_name` is "/bla/bla/servo" instead of "servo".
   */

  // Create a local copy of servo_name
  char servo_name_copy[1000];
  strcpy(servo_name_copy, servo_name);

  // Check whether local or global run by searching for '/' in `servo_name`
  char* servo_name_without_path = strrchr(servo_name_copy, '/');
  if (servo_name_without_path == NULL)
  { // It was a local run, therefore `servo_name` is simply "servo"

    sprintf(servo_name_copy,".%s_log",servo_name);

  } else
  { // It was a global run, take special measures

    // Get pointer to "servo" in "/bla/bla/servo" and cut the string into two
    *servo_name_without_path = 0;
    servo_name_without_path++;

    // Create place for "/.'servo'_log"
    char servo_name_without_path_log[100];

    // Create "/.servo_log"
    sprintf(servo_name_without_path_log, "/.%s_log", servo_name_without_path);

    // Concat "/bla/bla" and "/.servo"
    strcat(servo_name_copy, servo_name_without_path_log);

  }

  // Save log information
  FILE *fd = fopen(servo_name_copy,"w");
