...
initCollectData( int freq )
{
  ...
  addToMan("saveDataCompact","save data from data collection to file",saveDataCompact);
...
/*!*****************************************************************************
 *******************************************************************************
\note  reinitCollectData
\date  March 2014

\remarks

        reinitialized the data collection package
...

{
  ...
  if (fwrite(&(buffer[1][1]),sizeof(float),n_data_in_buffer*n_cols,fp)!= n_data_in_buffer*n_cols) {
    printf( "cannot fwrite matrix.\n" );
    return;
  }
