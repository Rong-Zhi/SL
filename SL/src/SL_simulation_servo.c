...
static void clearRobotMessages(void);
...
init_simulation_servo(void)
{
  ...
  addToMan("rt","toggle real-time processing",toggleRealTime);  
...
//  semGive(sm_motor_servo_sem);
...
#if defined(__XENO__) || defined(__LINUX_RT__)
  struct timespec t, t_sleep;

  clock_gettime(CLOCK_MONOTONIC,&t);
  current_time = (double) t.tv_sec + ((double)t.tv_nsec)/1.e9;
  if (real_time) {
	  double delta_time = 1./(double)simulation_servo_rate - (current_time - last_time);
	  if (delta_time < 0){
		  delta_time = 0;
	  }
	  else {
		  t_sleep.tv_sec = 0;
		  t_sleep.tv_nsec = delta_time*1.e9;
		  clock_nanosleep(CLOCK_MONOTONIC, 0, &t_sleep, NULL);
	  }
	  clock_gettime(CLOCK_MONOTONIC,&t);
	  current_time += delta_time;
  }

#else //Old linux and mac
  struct timeval t;
  gettimeofday(&t,NULL);
  ...
  if (real_time) {
	  ...
	  if (delta_time < 0){
		  ...
		  taskDelay(ns2ticks(delta_time*1.e9));
	  }
	  current_time += delta_time;
  }
#endif
...
checkForMessages(void)
{
  ...
  for (k=1; k<=sm_simulation_message->n_msgs; ++k) {
    ...
    // -------------------------------------------------------------------------
      ...
      memcpy(&(buf[1]),sm_simulation_message->buf+sm_simulation_message->moff[k], sizeof(float)*(3*n_dofs));
    ...
    // ---------------------------------------------------------------------------
    }else if (strcmp(name, "changePDGains") == 0) {
    	float buf[n_dofs*2+1];
		memcpy(&(buf[1]), sm_simulation_message->buf+sm_simulation_message->moff[k], sizeof(float) * (2 * n_dofs));
		for (i = 1; i <= n_dofs; ++i) {
			controller_gain_th[i] = (double) buf[i];
			controller_gain_thd[i] = (double) buf[i + n_dofs];
		}
...
clearRobotMessages(void)
{
  ...
  // check whether a message is available
  if (semTake(sm_robot_message_ready_sem,NO_WAIT) == ERROR)
    return;

  // receive the message
  if (semTake(sm_robot_message_sem,ns2ticks(TIME_OUT_NS)) == ERROR) {  
    printf("Couldn't take robot message semaphore\n");
    return;
  }
   
  // give back semaphore
  sm_robot_message->n_msgs = 0;
  sm_robot_message->n_bytes_used = 0;
  semGive(sm_robot_message_sem);
