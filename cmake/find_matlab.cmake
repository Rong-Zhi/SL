...
# Modified by Alexandros Paraschos
# Detects any Matlab version in Unix systems if installed in the
# /usr/local or /opt locations. 
# If multiple versions exist then it selecets the latest one as default.
...
ELSE(WIN32)
  ...
  IF(APPLE)
    ...
    FIND_PATH(MATLAB_ROOT "trademarks.txt" ${MATLAB_PATHS})
  ...
  ELSE(APPLE)
    ...
    FILE(GLOB MATLAB_OPT_PATHS_BIS "/opt/[Mm][Aa][Tt][Ll][Aa][Bb]/R*")
    FILE(GLOB MATLAB_USR_PATHS "/usr/[Mm][Aa][Tt][Ll][Aa][Bb]*")

    SET(MATLAB_PATHS ${MATLAB_LOCAL_PATHS} ${MATLAB_LOCAL_PATHS_BIS} ${MATLAB_OPT_PATHS} ${MATLAB_OPT_PATHS_BIS} ${MATLAB_USR_PATHS})

    IF( NOT "${MATLAB_PATHS}" STREQUAL "" )
        LIST(SORT MATLAB_PATHS)
        LIST(REVERSE MATLAB_PATHS)
    ENDIF()
...
IF(APPLE)
  ...
  ELSE(MATLAB_BOOST_FILESYSTEM_LIBRARY)
    # Fix for MATLAB_R2015b
    FIND_FILE(MATLAB_BOOST_FILESYSTEM_LIBRARY "libboost_filesystem.dylib" ${MATLAB_LIBRARIES_PATHS})
      IF(MATLAB_BOOST_FILESYSTEM_LIBRARY)
          SET(
              MATLAB_LIBRARIES
              ${MATLAB_LIBRARIES}
